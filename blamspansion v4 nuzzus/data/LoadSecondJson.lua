function onCreate()
    ...
    runHaxeCode([[
        ...
        for (Section in Notedata)
		{
			for (songNotes in Section.sectionNotes)
			{
              ...
              var OpponentNoteSing= Section.mustHitSection;
              if (songNotes[1] > 3)
              {
                OpponentNoteSing= !Section.mustHitSection;
                MustHitSection = !Section.mustHitSection;
              }
              var LastNote:Note;
              if (game.unspawnNotes.length > 0) LastNote = game.unspawnNotes[Std.int(game.unspawnNotes.length - 1)];
              else LastNote = null;
              var NewNote:Note = new Note(Strum, NoteData, LastNote, false, 0, [0]);
              NewNote.mustPress = MustHitSection;
              NewNote.opponentSing = OpponentNoteSing;
              ...
              var floor:Int = Math.floor(Length);
                if(floor > 0) {
                    for (susNote in 0...floor+1)
                    {
                        ...
                        var NewSustan:Note = new Note(Strum + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(]]..getProperty('songSpeed')..[[, 2)), NoteData, LastNote, true, 0, [0]);
                        NewSustan.opponentSing = OpponentNoteSing;
						...
						if (NewSustan.mustPress)
						{
							NewSustan.x += FlxG.width / 2; // general offset
						}
				...
				{
					NewNote.x += FlxG.width / 2; // general offset
				}
				else if(ClientPrefs.middleScroll)
				{
					NewNote.x += 310;
    ...
    NewsCharacter={}
    CreateCharacter('fabs','fabs',326-110, 232+460,'Opponent2')
    CreateCharacter('biddle',"biddle",1294+210, 102+450,'Player2')
...
function onCreatePost()
    ...
    for i=0,getProperty('unspawnNotes.length')-1 do
        if getPropertyFromGroup('unspawnNotes',i,'noteType')=='SecondJson' then
            ...
            if (characterPlayingAs == 2) then
                setPropertyFromGroup('unspawnNotes', i, 'blockHit', true)
            else
                if  getPropertyFromGroup('unspawnNotes',i,'opponentSing') then
                    setPropertyFromGroup('unspawnNotes', i, 'blockHit', true)
                else
...
function onUpdate(elapsed)
    ...
    if ActualNoteId~=nil then
        if (characterPlayingAs == 2) then
            if not GetFromNote(ActualNoteId,'opponentSing') and GetFromNote(ActualNoteId,'canBeHit') then
            ...
            if GetFromNote(ActualNoteId,'opponentSing') and GetFromNote(ActualNoteId,'canBeHit') then
                if GetFromNote(ActualNoteId,'isSustainNote') then
            ...
            if not GetFromNote(ActualNoteId,'opponentSing') and GetFromNote(ActualNoteId,'wasGoodHit') then
                HitbyOpponent(ActualNoteId)
            end
            if GetFromNote(ActualNoteId,'opponentSing') and GetFromNote(ActualNoteId,'canBeHit') then
                if GetFromNote(ActualNoteId,'isSustainNote') then
