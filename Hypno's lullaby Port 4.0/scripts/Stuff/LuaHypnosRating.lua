...
function GetRating(RatingScore)
    ...
    return tag..EarlyLater..getProperty(string.lower(tag)..'s')
end
local Negative=0
function Popup(noteID,Miss)
    ...
    if LuaShowRating then
        if stringStartsWith(PopTag,'Sick') then
            if getProperty('goods')<=0 and getProperty('bads')<=0 and getProperty('shits')<=0 and misses<=0 then
        ...
        if getPropertyFromClass('PlayState','isPixelStage') then
            if songPath=='brimstone' then
                scaleObject(PopTag,getPropertyFromClass('PlayState','daPixelZoom')* 0.7*0.5,getPropertyFromClass('PlayState','daPixelZoom')* 0.7*0.5)
            else
                scaleObject(PopTag,getPropertyFromClass('PlayState','daPixelZoom')* 0.7,getPropertyFromClass('PlayState','daPixelZoom')* 0.7)
            end

            setProperty(PopTag..'.antialiasing',false)
        else
