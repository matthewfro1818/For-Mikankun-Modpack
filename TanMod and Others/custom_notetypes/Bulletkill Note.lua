function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Bulletkill Note' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'accelerantHank/bullet_notes'); --Change texture
			setPropertyFromGroup('unspawnNotes', i, 'missHealth', 10.6); --Change amount of health to take when you miss like a fucking moron
		end
	end
...
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'Bullet Note' then
		...
		-- health loss | || || |_
		--setProperty('health', getProperty('health') - 9.9);
...
function onUpdate(elapsed)
	if healthDrain > 0 then
		healthDrain = healthDrain - 9.9 * elapsed;
		setProperty('health', getProperty('health') - 9.9 * elapsed);
		if healthDrain < 0 then
			healthDrain = 0;
		end
	end
