...
function onBeatHit()
	...
	doTweenY("themotherfucking2",'stageback',1000,5,'quintOut')
		characterPlayAnim('girlfriend', 'shit', true)
	...
	if curBeat == 350 then
	doTweenY("themotherfucking",'space', 800, -100,"linear")
	doTweenY("themotherfucking2",'stageback', -140, -500,'quintOut')
		 setProperty('defaultCamZoom',0.5)
	 	 doTweenZoom('WHEEE','camGame',0.5,1.5,'quintInOut')
	end
objectPlayAnimation('loaf', 'bop', true)
	...
	if curBeat >= 350 and curBeat < 400 then
		  cameraShake('camGame',0.02,1.2)
        cameraShake('camHUD',0.001,1.2)
	end
